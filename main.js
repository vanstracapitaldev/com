...
function showAdminLogin() {
            ...
            <div id="adminLoginStep1">
                <form id="adminCredentialsForm">
                    <div class="mb-4">
                        ...
                        <input type="email" id="adminEmail" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gold-accent" placeholder="admin@vanstra.bank" value="admin@vanstra.bank">
                    </div>
                    <div class="mb-4">
                    ...
                    <div class="flex space-x-4">
                        ...
                        <button type="submit" class="flex-1 px-4 py-2 bg-gold-accent text-white rounded-md hover:bg-opacity-90">Next</button>
                    </div>
                </form>
            </div>
            <div id="adminLoginStep2" class="hidden">
                <form id="adminOTPForm">
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-2">OTP Code</label>
                        <input type="text" id="adminOTP" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gold-accent" placeholder="Enter OTP code" maxlength="6">
                        <p class="text-xs text-gray-500 mt-2">Check your email for the OTP code</p>
                    </div>
                    <div id="adminOTPError" class="mb-4 text-red-600 text-sm hidden"></div>
                    <div class="flex space-x-4">
                        <button type="button" id="backBtn" class="flex-1 px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50">Back</button>
                        <button type="submit" class="flex-1 px-4 py-2 bg-gold-accent text-white rounded-md hover:bg-opacity-90">Verify</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    let adminTempToken, adminEmail;
    
    // Step 1: Credentials
    document.getElementById('adminCredentialsForm').addEventListener('submit', async function(e) {
        e.preventDefault();
        ...
        try {
            ...
            errorEl.textContent = error.message || 'Login failed. Please check credentials.';
            errorEl.classList.remove('hidden');
    ...
    document.getElementById('adminOTPForm').addEventListener('submit', async function(e) {
        ...
        if (!otp) {
            errorEl.textContent = 'Please enter OTP code';
            errorEl.classList.remove('hidden');
            return;
        }
        ...
        try {
            ...
            errorEl.textContent = error.message || 'OTP verification failed. Please try again.';
            errorEl.classList.remove('hidden');
            submitBtn.disabled = false;
